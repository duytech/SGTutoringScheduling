...
public class TutorLoadServiceTests : SqlServerFixture
{
    private TutorLoadService Service => NewTutorLoadService();

    [Fact]
    public async Task GetForDayAsync_WithNoBookings_ReturnsEmptyLoadsForThatDate()
    {
        var loads = await Service.GetForDayAsync(new DateOnly(2026, 3, 6));
    ...
    [Fact]
    public async Task GetForDayAsync_CountsBookedLessonsAndReportsTheLimit()
    {
        Add(
            BookingFactory.Create("A", start: "08:00", roomId: "R1"),
            BookingFactory.Create("B", start: "10:00", roomId: "R2"));

        var load = Assert.Single((await Service.GetForDayAsync(new DateOnly(2026, 3, 6))).TutorLoads);
    ...
    [Fact]
    public async Task GetForDayAsync_FlagsATutorOverTheDailyLimit()
    {
    ...
    [Fact]
    public async Task GetForDayAsync_IgnoresCancelledLessons()
    {
        Add(
            BookingFactory.Create("A", start: "08:00", roomId: "R1"),
